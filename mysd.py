...
if __name__ == '__main__':
    ...
    with open(f'mutual_info_{model_name[:-8]}.toml', 'r') as f:
        mutual_info_dict = toml.load(f)
    with open(f'transfer_entropy_{model_name[:-8]}.toml', 'r') as f:
        entropy_dict = toml.load(f)

    mi_score = [float(value) for value in mutual_info_dict.values()]
    te_score = [float(value) for value in entropy_dict.values()]
    score = []
    for mi_s, te_s in zip(mi_score, te_score):
        if mi_s < 0.2 and te_s > 0.06:
            score.append(1.0)
        else:
            score.append(0.0)
    score = te_score
    ...
    with torch.no_grad():
        ...
        acc_filename = 'transfer_entropy_selected_dropout_acc.toml'
